...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.prune:
        attn_pattern = np.load(args.attn_path) #(num_layers, 1, num_attention_heads, max_seq_length, max_seq_length)
        #use args.tau as a percentage; find corresponding magnitude-threshold for each layer and calculate prune mask
        prune_mask = []
        for layer_attn_pattern in attn_pattern:
            ...
            prune_mask.append( layer_attn_pattern  <=  threshold) #1 if we want to prune
        prune_mask = np.array(prune_mask)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        output_eval_file = os.path.join(eval_output_dir, prefix, "eval_results_{}.txt".format(args.tau))
        write_results = f"{args.tau} {args.seed} "
        ...
        write_results += '\n'
        with open(f"{args.output_dir[6:-1]}.txt", "a") as file:
...
class DataProcessingArguments:
    ...
    #--------------------------------------------------------------------------------------------------------------------------------
    save_attention_patterns: bool = field(
        default=False, metadata={"help": "Save Attention Pattern To Disk"})

    prune: bool = field(
        default=False, metadata={"help": "Load Attention Pattern from Disk and Prune"})

    attn_path: str = field(
        default=None, metadata={"help": "path to attention pattern; to be used for either saving or loading them "}
    )

    tau: float = field(
        default=0,
        metadata={"help": "prune percentage"}
    )

    debug: bool = field(
        default=False, metadata={"help": "Print"})
